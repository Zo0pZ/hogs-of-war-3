...
/* ================= DATA ================= */
const NATIONS=[
 {id:'GB',name:'Great Britain',team:"Tommy's Trotters",helm:0x4c5a31,flag:['#b7332c','#e8e0c8','#31456e'],names:['Nigel','Winston','Reg','Bertie','Clive','Percy','Alfie','Monty'],perk:'Tea logistics restore morale and grant a free medic kit at the start of battle.',taunt:'The stiff-upper snout is not amused by your swill-lust.'},
 {id:'FR',name:'France',team:'Garlic Grunts',helm:0x35507e,flag:['#31456e','#e8e0c8','#b7332c'],names:['Pierre','Jacques','Henri','Marcel','Gaston','Claude','Antoine','Hugo'],perk:'Tactical lunch breaks mean every medic squad begins with 1 extra bandage and better heal uptime.',taunt:'A properly timed snack can be just as lethal as a grenade.'},
 {id:'DE',name:'Germany',team:'Sow-A-Krauts',helm:0x4a5155,flag:['#2b2b2b','#b7332c','#d8a63c'],names:['Hans','Fritz','Otto','Klaus','Dieter','Wolfgang','Gunther','Ludwig'],perk:'Precision engineering: all engineers bring extra mines and pack a stronger blast profile.',taunt:'The sausage machine has not broken down; it is merely warming up.'},
 {id:'SU',name:'Soviet Union',team:'Piggystroika',helm:0x6b3428,flag:['#b7332c','#b7332c','#d8a63c'],names:['Boris','Ivan','Dmitri','Sergei','Yuri','Nikolai','Vlad','Igor'],perk:'Inexhaustible reserves of angry mean your survivors can be rebuilt into fresh grunts at no extra cost.',taunt:'There are no shortages in the Red Sty, only stubbornness.'},
 {id:'US',name:'USA',team:"Uncle Ham's Hogs",helm:0x5e5239,flag:['#31456e','#e8e0c8','#b7332c'],names:['Hank','Buck','Tex','Chuck','Earl','Buster','Woody','Gus'],perk:'Biggest bazookas in the archipelago: all heavy weapons and shells hit with a heavier punch.',taunt:'Good news, everybody: we brought more explosives than your whole family.'},
 {id:'JP',name:'Japan',team:'Sushi-Swine',helm:0x736a3c,flag:['#e8e0c8','#b7332c','#e8e0c8'],names:['Kenji','Hiro','Taro','Yuki','Goro','Riku','Osamu','Kazu'],perk:'Never retreat. Every squad begins battle with a little extra morale and their first wound is never the last.',taunt:'The enemy may retreat; the Sushi-Swine will simply advance more loudly.'},
];
const CLASSES=[
 {id:'Heavy',blurb:'+25% blast damage', officer:'Artillery Officer — off-map barrages'},
 {id:'Medic',blurb:'Regenerates; mighty medikits', officer:'Surgeon-Major — battlefield resurrection (once!)'},
 {id:'Spy',blurb:'Deadeye shots; ignores mines', officer:'Spymaster — disguise as enemy hog'},
 {id:'Engineer',blurb:'Extra mines; blast-hardened', officer:'Sapper-Colonel — deploy & crew vehicles'},
];
const RANKS=['Grunt','Private','Corporal','Sergeant','Major','Hero of the Sty'];
...
const CLASS_SPECIAL={Heavy:'sowzooka',Medic:'hospital',Spy:'tele',Engineer:'girder'};
...
const WEAPONS=[
 {id:'rifle',  name:'Rifle',   key:'1', ammo:Infinity, kind:'ray', range:120, dmg:16},
 {id:'bazooka',name:'Bazooka', key:'2', ammo:Infinity, kind:'shell', r:7, dmg:45, wind:true},
 {id:'grenade',name:'Grenade', key:'3', ammo:4, kind:'bounce', fuse:3, r:6.4, dmg:40},
 {id:'cluster',name:'Cluster', key:'4', ammo:2, kind:'bounce', fuse:3, r:5, dmg:30, cluster:true},
 {id:'mg',     name:'Machine Gun',key:'5', ammo:3, kind:'burst', range:90, dmg:9, shots:8, spread:0.045},
 {id:'flame',  name:'Flamethrower',key:'6', ammo:2, kind:'flame', range:26, dmg:7, ticks:16, cone:0.30},
 {id:'sniper', name:'Sniper',  key:'7', ammo:2, kind:'ray', range:400, dmg:55},
 {id:'mine',   name:'Mine',    key:'8', ammo:3, kind:'minetoss', r:6.6, dmg:45},
 {id:'medikit',name:'Medikit', key:'9', ammo:2, kind:'heal', heal:35},
 {id:'arty',   name:'Artillery',key:'0', ammo:1, kind:'howitzer', shells:3, r:11, dmg:58, wind:true, spread:0.045},
 // Bomber sits last: selecting it swings the camera to the bombsight, so it
 // shouldn't be something you pass through while cycling weapons
 {id:'strike', name:'Bomber',  key:'-', ammo:1, kind:'strike', bombs:5, r:6.2, dmg:36},
 // ---- class specials: only the matching hog sees these, so they share a key ----
 {id:'sowzooka',name:'Sowzooka', key:'=', ammo:1, kind:'shell', r:16, dmg:78, wind:true, classOnly:'Heavy'},
 {id:'hospital',name:'Field Hospital',key:'=', ammo:1, kind:'squadheal', heal:40, classOnly:'Medic'},
 {id:'tele',    name:'Teleport',  key:'=', ammo:2, kind:'tele', classOnly:'Spy'},
 {id:'girder',  name:'Girder',    key:'=', ammo:3, kind:'build', classOnly:'Engineer'},
 // ---- vehicle & emplacement weapons: only offered when you're crewing one ----
 {id:'tankgun', name:'Tank Gun',  key:'1', ammo:Infinity, kind:'shell', r:13, dmg:72, crewOnly:'tank'},
 {id:'empgun',  name:'Emplaced Gun',key:'1', ammo:Infinity, kind:'howitzer', shells:2, r:12, dmg:62, wind:true, spread:0.04, crewOnly:'arty'},
 {id:'empmg',   name:'Nest MG',   key:'1', ammo:Infinity, kind:'burst', range:95, dmg:11, shots:10, spread:0.04, crewOnly:'mg'},
 // naval: the boat gun adds to your kit, the destroyer's main battery replaces it
 {id:'boatgun', name:'Boat Gun',  key:'[', ammo:Infinity, kind:'burst', range:95, dmg:10, shots:8, spread:0.05, crewOnly:'boat'},
 {id:'navalgun',name:'Naval Gun', key:'1', ammo:3, kind:'howitzer', shells:2, r:19, dmg:98, wind:true, spread:0.025, crewOnly:'ship'},
];
const REGIONS=[
 {island:'Spamshead', name:'Bristle Beach', theme:'beach', brief:"The beaches of Spamshead are hot, sticky, and full of enemy machine-gun nonsense. Land, hold, and keep the bristles on the right side of the shoreline."},
 {island:'Spamshead', name:'Prawn Point', theme:'beach', brief:"A long, flapping strip of beach where every hidden shell and every crossed wire turns into a proper comedy of errors. Advance with a grin and a mortar."},
 {island:'Spamshead', name:'Hedgehog Run', theme:'green', brief:"The land is hedged, muddy, and absolutely no use to anyone unless you can make it into a tactical picnic. Push through the hedgerows and don't get trapped in the lunch queue."},
 {island:'El Hamein Sands', name:'Sow Oasis', theme:'desert', brief:"The desert wakes up in a heat haze and leaves you guessing where the enemy ends and the mirage begins. Spam the dunes, keep the tanks alive, and don't trust any puddle."},
 {island:'El Hamein Sands', name:'Dune Slopper', theme:'desert', brief:"A sea of shifting sand with a few very unhappy pigs in it. One false move and the whole line slides into the wrong war."},
 {island:'El Hamein Sands', name:'Trotter Gap', theme:'desert', brief:"The tank debut arrives here in all its clanking glory. The best tactic is to both out-gun the other side and out-swear the engine when it refuses to start."},
 {island:'El Hamein Sands', name:'Oasis of Regret', theme:'desert', brief:"No one gets a proper rest in a swill war, not even in a place with water on the map. If the enemy's here, then so are the mines."},
 {island:'The Trotterlands', name:'Flooded Wallow', theme:'green', brief:"Half the map is swamp, half is bogeyed infantry, and all of it smells faintly of war and bad barn etiquette. Use the flood to your advantage, not your boots."},
 {island:'The Trotterlands', name:'Blown Bridge', theme:'green', brief:"A ruined crossing, one good bridge, and a million reasons not to stand on it. Send the engineers first before the whole platoon decides to walk in the same place."},
 {island:'The Trotterlands', name:'Commando Puddle', theme:'rock', brief:"A wet, moonlit raid zone where quiet approaches are best replaced by shocking amounts of noise. A surprise attack is only surprising if somebody survives it."},
 {island:'The Trotterlands', name:'Stinkford Ford', theme:'rock', brief:"A bad crossing, a worse plan, and a truly splendid opportunity for the enemy to get a bit of luck. Choose speed, not dignity."},
 {island:'Stygrad', name:'Rimegrit Square', theme:'snow', brief:"Curfew is a joke in a war zone, but a frozen square full of rubble is serious business. Keep the line tight and the snouts warm with shellfire."},
 {island:'Stygrad', name:'Frosted Avenue', theme:'snow', brief:"The city is falling apart in a way that suggests a war and a building firm both had a very bad day. Put the houses to work as cover, then make them regret it."},
 {island:'Stygrad', name:'Black Sausage Hollow', theme:'snow', brief:"A siege with all the obvious hazards and none of the charm. Sentimentality will get you flattened; engineering will get you a trench and a laugh."},
 {island:'Stygrad', name:'The Shivering Citadel', theme:'dark', brief:"The enemy has dug into the last standing walls and is somehow still using a shovel and a grudge. Break the line, then break the morale."},
 {island:'Stygrad', name:'Lardfall Courtyard', theme:'dark', brief:"Urban ruin, attritional fire, and a building that looks as if it was personally insulted by the war. Keep the pace up and the medics dry."},
 {island:'Mount Sowribachi', name:'Ashen Ladder', theme:'rock', brief:"A volcanic island with all the natural beauty of a blown-up biscuit tin. Climb, hold, and make life very unpleasant for anyone standing on the high ground."},
 {island:'Mount Sowribachi', name:'Bunker Spur', theme:'rock', brief:"Fortified, steep, and utterly committed to disagreeing with your plan. It wants to fight at close range, so make it choose between cover and comedy."},
 {island:'Mount Sowribachi', name:'Molten Ditch', theme:'dark', brief:"The heat is a weapon and so is the siren song of the enemy's artillery. This is not the place to be sentimental about your boots."},
 {island:'Mount Sowribachi', name:'Sowribachi Ridge', theme:'rock', brief:"The ridge is high, narrow, and likely to end in everyone shouting. Take it with a combination of nerves and happily excessive explosives."},
 {island:'Schweinfurt Redoubt', name:'Redoubt Gate', theme:'dark', brief:"The enemy fortress presents itself like a city-sized obstacle course run by a disgruntled butcher. Enter carefully, and leave loudly."},
 {island:'Schweinfurt Redoubt', name:'Sausage Bastion', theme:'dark', brief:"A fortress that combines every embarrassing idea in the war into one tremendous pile of steel, shrapnel, and pride. You will need everything, including a sense of humour."},
 {island:'Schweinfurt Redoubt', name:'Boomslab Heights', theme:'dark', brief:"Victory depends on a tidy battle of cover, arty, and the willingness to use good old-fashioned overkill. The pigs have earned it."},
 {island:'Schweinfurt Redoubt', name:'Fryline Keep', theme:'dark', brief:"Every one of your training drills has been leading here. The walls are thick, the fire is heavy, and the swill is waiting somewhere behind all that nonsense."},
 {island:'Schweinfurt Redoubt', name:'Last Swill Gate', theme:'dark', brief:"The final objective is not a position but an argument conducted with shells. Take the gate, keep the platoon together, and do not be polite about the last few yards."},
];
const THEMES={
...
const Q={
 turn:["Right, let's be having you!","Once more unto the bacon!","Watch and learn, piglets.","Somebody's getting crackled today.","Time to bring home the bacon!","Steady... steady..."],
 hurt:["Right in the chops!","Medic! MEDIIIC!","I'm a hog, not a target!","Ow! Me crackling!","That stings, you swine!","Mummy..."],
 kill:["Sizzled!","That's him porked.","Chalk one up for the sty!","Hope he liked fireworks!","Crackling for everyone!"],
 death:["Tell the sty... I loved her...","Avenge... my breakfast...","I regret... nuffink!","Not the trotters..."],
 drown:["I never learned to swiiiim!","Blub. Blub.","Who ordered soup?!"],
 miss:["Who put that hill there?!","Blast! Next time.","Me aim's gone all wonky.","I meant to do that."],
 fall:["Ooof! Me bacon!","I felt that in me chops!"],
 heal:["Lovely jubbly!","Good as new-ish!","That's the stuff."],
};
...
function makeLandingCraft(){
  ...
  const hullM=new THREE.MeshStandardMaterial({color:0x4b5340,roughness:0.72,metalness:0.35,flatShading:true});
  const deckM=new THREE.MeshStandardMaterial({color:0x5d6450,roughness:0.85,flatShading:true});
  const trimM=new THREE.MeshStandardMaterial({color:0x39402f,roughness:0.7,metalness:0.4,flatShading:true});
  const L=8.4, Wd=4.2;
  const hull=new THREE.Mesh(new THREE.BoxGeometry(L,1.5,Wd),hullM);
  hull.position.y=0.35; g.add(hull);
  const deck=new THREE.Mesh(new THREE.BoxGeometry(L-1.2,0.22,Wd-0.9),deckM);
  deck.position.y=1.06; g.add(deck);
  for(const s of [-1,1]){                                   // gunwales
    const w=new THREE.Mesh(new THREE.BoxGeometry(L-0.8,1.15,0.34),hullM);
    w.position.set(0,1.5,s*(Wd/2-0.2)); g.add(w);
  }
  const stern=new THREE.Mesh(new THREE.BoxGeometry(0.4,1.5,Wd-0.5),hullM);
  stern.position.set(-L/2+0.3,1.5,0); g.add(stern);
  const ramp=new THREE.Mesh(new THREE.BoxGeometry(2.4,0.3,Wd-0.7),trimM);  // bow ramp, down
  ramp.position.set(L/2+0.7,0.55,0); ramp.rotation.z=0.32; g.add(ramp);
  const house=new THREE.Mesh(new THREE.BoxGeometry(1.5,1.3,1.6),trimM);
  house.position.set(-L/2+1.5,2.2,0); g.add(house);
  const mast=new THREE.Mesh(new THREE.CylinderGeometry(0.09,0.09,2.1,6),trimM);
  mast.position.set(-L/2+1.5,3.6,0); g.add(mast);
...
function boardBoat(h){
  ...
  bubble(h,pick(['All aboard!','Cast off!','Anchors aweigh!']));
  sfx('beep');
  return true;
}
...
function makeDestroyer(){
  const g=new THREE.Group();
  const hullM=new THREE.MeshStandardMaterial({color:0x555f63,roughness:0.7,metalness:0.45,flatShading:true});
  const deckM=new THREE.MeshStandardMaterial({color:0x4a5257,roughness:0.85,flatShading:true});
  const trimM=new THREE.MeshStandardMaterial({color:0x353c40,roughness:0.7,metalness:0.5,flatShading:true});
  const L=34, W=6.4;
  const hull=new THREE.Mesh(new THREE.BoxGeometry(L,4.2,W),hullM);
  hull.position.y=1.4; g.add(hull);
  const bow=new THREE.Mesh(new THREE.ConeGeometry(W/2,7,4),hullM);
  bow.rotation.z=-Math.PI/2; bow.rotation.y=Math.PI/4;
  bow.position.set(L/2+2.6,1.4,0); g.add(bow);
  const deck=new THREE.Mesh(new THREE.BoxGeometry(L,0.4,W-0.5),deckM);
  deck.position.y=3.6; g.add(deck);
  // superstructure and funnels
  const bridge=new THREE.Mesh(new THREE.BoxGeometry(7,3.2,4.4),deckM);
  bridge.position.set(-1,5.3,0); g.add(bridge);
  const wheel=new THREE.Mesh(new THREE.BoxGeometry(4,1.8,3.6),trimM);
  wheel.position.set(-1,7.6,0); g.add(wheel);
  for(const fx of [-7.5,3.5]){
    const funnel=new THREE.Mesh(new THREE.CylinderGeometry(1.05,1.25,3.4,10),trimM);
    funnel.position.set(fx,6.6,0); g.add(funnel);
  }
  const mast=new THREE.Mesh(new THREE.CylinderGeometry(0.14,0.14,6,6),trimM);
  mast.position.set(-1,11.2,0); g.add(mast);
  // main battery fore and aft
  for(const gx of [11.5,-11.5]){
    const turret=new THREE.Mesh(new THREE.CylinderGeometry(1.9,2.2,1.7,10),trimM);
    turret.position.set(gx,4.7,0); g.add(turret);
    for(const s of [-0.55,0.55]){
      const barrel=new THREE.Mesh(new THREE.CylinderGeometry(0.24,0.28,6.4,8),trimM);
      barrel.rotation.z=-Math.PI/2;
      barrel.position.set(gx+(gx>0?4.2:-4.2),5.0,s);
      g.add(barrel);
    }
  }
  for(const s of [-1,1]){                       // railings
    const rail=new THREE.Mesh(new THREE.BoxGeometry(L,0.5,0.16),trimM);
    rail.position.set(0,4.1,s*(W/2-0.4)); g.add(rail);
  }
  g.traverse(o=>{ if(o.isMesh){ o.castShadow=true; o.receiveShadow=true; } });
  return g;
}
...
function boardShip(h){
  ...
  bubble(h,pick(['Permission to come aboard!','Main battery, stand by!','Now this is artillery.']));
  speak('Main battery, stand by!');
...
function makeTank(){
  const g=new THREE.Group();
  const hullM=new THREE.MeshStandardMaterial({color:0x4a5340,roughness:0.68,metalness:0.45,flatShading:true});
  const darkM=new THREE.MeshStandardMaterial({color:0x2f352a,roughness:0.8,metalness:0.35,flatShading:true});
  const hull=new THREE.Mesh(new THREE.BoxGeometry(7.4,1.5,4.2),hullM);
  hull.position.y=1.5; g.add(hull);
  const glacis=new THREE.Mesh(new THREE.BoxGeometry(2.0,1.2,4.2),hullM);
  glacis.position.set(3.3,1.75,0); glacis.rotation.z=-0.42; g.add(glacis);
  for(const s of [-1,1]){                                   // tracks
    const tr=new THREE.Mesh(new THREE.BoxGeometry(7.8,1.25,1.05),darkM);
    tr.position.set(0,0.85,s*2.1); g.add(tr);
    for(let i=0;i<5;i++){
      const wheel=new THREE.Mesh(new THREE.CylinderGeometry(0.52,0.52,0.5,10),darkM);
      wheel.rotation.x=Math.PI/2; wheel.position.set(-2.9+i*1.45,0.85,s*2.1); g.add(wheel);
    }
  }
  const turret=new THREE.Group();
  const tur=new THREE.Mesh(new THREE.CylinderGeometry(1.75,2.05,1.35,9),hullM);
  tur.position.y=2.9; turret.add(tur);
  const mantlet=new THREE.Mesh(new THREE.BoxGeometry(0.7,0.8,1.2),darkM);
  mantlet.position.set(1.7,2.9,0); turret.add(mantlet);
  const barrel=new THREE.Mesh(new THREE.CylinderGeometry(0.24,0.28,5.2,10),darkM);
  barrel.rotation.z=-Math.PI/2; barrel.position.set(4.4,2.9,0); turret.add(barrel);
  const cupola=new THREE.Mesh(new THREE.CylinderGeometry(0.55,0.55,0.45,8),hullM);
  cupola.position.set(-0.6,3.75,0.4); turret.add(cupola);
...
function boardTank(h){
  ...
  bubble(h,pick(['Start her up!','Right, this is more like it.','Mind the paintwork.']));
  sfx('beep'); B.sel='1'; buildTray();
  return true;
}
...
function makePig(helmColor){
  ...
  const helm=new THREE.Mesh(new THREE.SphereGeometry(0.72,16,10,0,Math.PI*2,0,Math.PI*0.52),
    new THREE.MeshStandardMaterial({color:helmColor,roughness:0.55,metalness:0.25}));
  helm.position.set(0.95,1.52,0); helm.scale.set(1.05,0.9,1.05); g.add(helm);
  const brim=new THREE.Mesh(new THREE.TorusGeometry(0.72,0.07,8,20),
    new THREE.MeshStandardMaterial({color:helmColor,roughness:0.35,metalness:0.65}));
  brim.rotation.x=Math.PI/2; brim.position.set(0.95,1.52,0); brim.scale.set(1.05,1.05,1); g.add(brim);
  g.traverse(o=>{ if(o.isMesh){ o.castShadow=true; } });
...
function fire(h,w,dir,speed){
  ...
  if(w.kind==='howitzer'){       // field gun: wheels up beside the hog and lobs heavy shells
    ...
    bubble(h,shipboard?'Main battery, fire!':'Bring up the big gun!');
    speak(shipboard?'Main battery, fire!':'Bring up the big gun!');
...
function makePlaneMesh(){
  const g=new THREE.Group();
  const mat=new THREE.MeshStandardMaterial({color:0x4a4636,roughness:0.6});
  const body=new THREE.Mesh(new THREE.CylinderGeometry(0.7,0.5,6,10),mat);
  body.rotation.z=-Math.PI/2; g.add(body);
  const wing=new THREE.Mesh(new THREE.BoxGeometry(1.8,0.16,9),mat); wing.position.x=0.4; g.add(wing);
  const tail=new THREE.Mesh(new THREE.BoxGeometry(0.9,1.3,0.14),mat); tail.position.set(-2.8,0.55,0); g.add(tail);
  const tw=new THREE.Mesh(new THREE.BoxGeometry(0.8,0.14,2.6),mat); tw.position.set(-2.8,0.3,0); g.add(tw);
  g.traverse(o=>{ if(o.isMesh) o.castShadow=true; });
...
function planAIBoard(h){
  ...
  // Once a hog has set off for something it keeps going. Re-rolling the decision
  // every turn meant it never covered the distance on the larger maps and simply
  // dithered near the spawn instead.
  if(h.aiWant){
    const o=h.aiWant.o;
    const gone=o.dead||o.rider||(o.shots!==undefined&&(o.shots<=0||empManned(o)));
    if(gone) h.aiWant=null;
    else return {o,range:h.aiWant.range,
                 time:Math.min(3.4,Math.hypot(o.x-h.position.x,o.z-h.position.z)/9+0.5)};
  }
  ...
  const REACH=95;                      // several turns' march; it commits and keeps going
  const cand=[];
  const add=(o,worth,range)=>{
    ...
    cand.push({o,range,sc:worth-d*0.28+Math.random()*4,d});
  };
  ...
  h.aiWant={o:best.o,range:best.range*0.85};
  return {o:best.o,range:best.range*0.85,time:Math.min(3.4,best.d/9+0.5)};
}
...
function planAI(h){
  ...
  if(h.cls==='Heavy'&&am.sowzooka>0) types.push(W('sowzooka'));   // the class special
  ...
  // no decent arc and no clean shot? call in the bombers. Aim at the target's
  // feet so fire()'s ground raycast lands the run right on top of them.
...
function buildAssetTags(){
  ...
  for(const t of tanks) add(t,'▮ TANK','a-tank',5.2);
  for(const b of boats) add(b,'⛵ BOAT','a-boat',3.6);
  for(const s of ships) add(s,'⚓ DESTROYER','a-ship',12);
...
function pauseStopsTheWorld(){ return !false; }
function openPause(){
...
function update(){
  ...
  switch(B.state){
    ...
    case 'action': {
      ...
      if(B.fired){
        B.retreatT-=dt;
        if(B.retreatT<=0){ endAction(); break; }
      }
      if(isAI(B.team)){
        B.st+=dt;
        ...
        if(B.aiBoard&&B.st<B.aiMoveEnd){
          ...
          if(d<=B.aiBoard.range){ aiBoard(h,o); h.aiWant=null; B.aiBoard=null; B.aiMoveEnd=B.st; }
  ...
  if(myAction){
    ...
    if(mode!==hintIsBomber){
      ...
      $('hint').innerHTML =
        mode==='ship'      ? '<b>WASD</b> steer the destroyer &nbsp;·&nbsp; <b>'+(gpActive?'RT':'SPACE')+'</b> fire the main battery &nbsp;·&nbsp; <b>'+key+'</b> back to your boat'
      : mode==='alongside' ? '<b>'+key+'</b> climb aboard the destroyer &nbsp;·&nbsp; <b>WASD</b> to pull away'
      : mode==='tank'      ? '<b>WASD</b> drive &nbsp;·&nbsp; <b>'+(gpActive?'RT':'SPACE')+'</b> fire the main gun &nbsp;·&nbsp; <b>'+key+'</b> bail out'
      : mode==='boardTank' ? '<b>'+key+'</b> climb into the tank'
      : mode==='gun'       ? '<b>'+(gpActive?'RT':'SPACE')+'</b> fire the emplaced gun &nbsp;·&nbsp; <b>'+key+'</b> stand down'
      : mode==='manGun'    ? '<b>'+key+'</b> man the gun'
      : mode==='sail'  ? '<b>WASD</b> steer the landing craft &nbsp;·&nbsp; <b>'+key+'</b> go ashore'
      : mode==='board' ? '<b>'+key+'</b> board the landing craft'
...
$('btnNext').onclick=()=>{
  ...
  } else { disposeBattle(); showOnly('menu'); refreshContinue(); refreshRecordBtn(); }
};
...
genTerrain('green');
refreshContinue(); refreshRecordBtn();
resize();
